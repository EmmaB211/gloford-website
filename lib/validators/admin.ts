...
const optionalRelativeUrl = z.string().max(2048).refine(

  (val) => /^\/|^\.\/|^\.\.\//.test(val) && !/\s/.test(val),
  { message: "Must be a valid relative URL path" }
).nullable().optional().or(z.literal("")).transform((v) => v || null);
...
export const serviceAreaToggleSchema = z.object({
  id: slug,
  isActive: z.string().transform((v) => v === "true"),
});

export const serviceAreaDeleteSchema = z.object({
  id: slug,
});
