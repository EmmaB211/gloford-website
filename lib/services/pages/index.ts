...
import { buildUniquePageSlug, ensureCollectionSlug } from "@/lib/pages/slug";
...
export const createPage = createService({
  ...
  exec: async ({ input, tx }) => {
    const slug = await buildUniquePageSlug(ensureCollectionSlug("partner", input.slug || input.title));
...
export async function getPublishedCollectionPage(kind: PageCollectionKind, leafSlug: string) {
  try {
    return await getPublishedPageBySlug(toCollectionSlug(kind, leafSlug));
  } catch (error) {
    if (error instanceof NotFoundError) {
      return getPublishedPageBySlug(leafSlug);
    }
    throw error;
  }
}
