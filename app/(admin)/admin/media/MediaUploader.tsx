...
export function MediaUploader({ onUploaded }: { onUploaded?: (row: { id: string; url: string }) => void }) {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setProgress(null);
      if (row && typeof onUploaded === "function") {
        try { onUploaded(row as { id: string; url: string }); } catch {}
      }
      router.refresh();
    } catch (e) {
