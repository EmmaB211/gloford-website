...
export function PageForm({ initial, appendHtml, showPartnerStatus }: { initial?: Initial; appendHtml?: string | null; showPartnerStatus?: boolean }) {
  ...
  const [status, setStatus] = useState<"PUBLISHED" | "ARCHIVED">(initial?.status ?? "PUBLISHED");
  ...
  const submit = () => {
    ...
    start(async () => {
      try {
        const payloadBlocks = blocks.length > 0 ? blocks : htmlToBlocks(bodyHtml);
        if (isEdit) {
          await updatePageAction({ id: initial!.id!, title, slug, seoTitle, seoDesc, blocks: payloadBlocks, status });
        } else {
          await createPageAction(
            { title, slug, seoTitle, seoDesc, blocks: payloadBlocks, status },
  ...
  // Append external HTML (e.g., an uploaded PDF embed) when provided by a parent.
  useEffect(() => {
    if (!appendHtml) return;
    setBodyHtml((prev) => prev + appendHtml);
    ...
    <div className="grid min-w-0 gap-6 lg:grid-cols-[1fr_280px]">
      <div className="space-y-6">
        <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-card)] p-5 space-y-4">
          ...
          {showPartnerStatus ? (
            <Field label="Partner status" hint="Choose whether this partner is current or previous">
              <select value={status} onChange={(e) => setStatus(e.target.value as "PUBLISHED" | "ARCHIVED")} className={inputCls}>
                <option value="PUBLISHED">Current partner</option>
                <option value="ARCHIVED">Previous partner</option>
              </select>
            </Field>
          ) : null}
        </section>
        ...
        <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-card)] p-5 space-y-4">
          ...
          <p className="text-sm text-[var(--color-muted-fg)]">Add rich text, images, or partner-logo blocks to this page.</p>
