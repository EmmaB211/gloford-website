      ...
      <div className="space-y-6">
        ...
        <Card title="Socials">
          <div className="grid gap-3 md:grid-cols-2">
            {(
              [
                ["twitter", "Twitter / X", "twitterEnabled"],
                ["facebook", "Facebook", "facebookEnabled"],
                ["instagram", "Instagram", "instagramEnabled"],
                ["linkedin", "LinkedIn", "linkedinEnabled"],
                ["youtube", "YouTube", "youtubeEnabled"],
              ] as const
              ...
              <Field key={key} label={label}>
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    role="switch"
                    aria-checked={Boolean(state.socials[enabledKey as keyof typeof state.socials])}
                    onClick={() =>
                      setState((s) => ({
                        ...s,
                        socials: {
                          ...s.socials,
                          [enabledKey]: !Boolean(s.socials[enabledKey as keyof typeof s.socials]),
                        },
                      }))
                    }
                    className={`inline-flex h-8 w-14 items-center rounded-full transition ${
                      state.socials[enabledKey as keyof typeof state.socials] ? "bg-[var(--color-primary)]" : "bg-[var(--color-muted)]"
                    }`}
                  >
                    <span
                      className={`inline-block h-6 w-6 transform rounded-full bg-[var(--color-bg)] shadow transition ${
                        state.socials[enabledKey as keyof typeof state.socials] ? "translate-x-6" : "translate-x-1"
                      }`}
                    />
                  </button>
                </div>
