...
import { splitPartnerPagesBySelection } from "@/lib/services/pages/sections";
...
export default async function PartnersPage() {
  ...
  const [rows, testimonials, faqs] = await Promise.all([
    db.page.findMany({
      where: { status: { in: ["PUBLISHED", "ARCHIVED"] }, slug: { startsWith: config.prefix } },
      orderBy: [{ updatedAt: "desc" }],
      select: { id: true, slug: true, title: true, seoDesc: true, blocks: true, status: true },
    }),
    getActiveTestimonials(),
    getActiveFaqs("partnerships"),
  ]);

  const { currentPages: currentPartners, previousPages: previousPartners } = splitPartnerPagesBySelection(rows);
  const hasAnyPartners = rows.length > 0;
    ...
    <>
      ...
      <section className="w-full bg-[var(--color-bg)] px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-16">
          <div className="rounded-[2rem] border border-[var(--color-border)] bg-white/70 p-6 shadow-sm sm:p-8 lg:p-10">
            <div className="mb-8 flex items-center justify-between gap-4">
              ...
              <div className="h-1.5 flex-1 rounded-full bg-[var(--color-muted)]" />
            </div>

            {currentPartners.length > 0 ? (
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                {currentPartners.map((row, i) => {
              ...
              <p className="text-sm text-[var(--color-muted-fg)]">{t("noCurrentPartners")}</p>
            )}
          </div>

          <div className="rounded-[2rem] border border-[var(--color-border)] bg-[rgb(250_247_255)] p-6 shadow-sm sm:p-8 lg:p-10">
            <div className="mb-8 flex items-center justify-between gap-4">
              <ScrollReveal>
                <h2 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">
                  {t("previousPartners")}
            ...
            {previousPartners.length > 0 ? (
              ...
              <p className="text-sm text-[var(--color-muted-fg)]">{t("noPreviousPartners")}</p>
            )}
          </div>
