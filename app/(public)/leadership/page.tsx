...
const DEPARTMENT_ORDER = ["Board", "Leadership", "Staff"];

export default async function LeadershipPage() {
  const t = await getTranslations("public.leadership");
  const grouped = await getTeamMembersByDepartment();
  const departments = Object.entries(grouped).sort(([left], [right]) => {
    const leftIndex = DEPARTMENT_ORDER.indexOf(left);
    const rightIndex = DEPARTMENT_ORDER.indexOf(right);

    if (leftIndex !== -1 || rightIndex !== -1) {
      return (leftIndex === -1 ? DEPARTMENT_ORDER.length : leftIndex) - (rightIndex === -1 ? DEPARTMENT_ORDER.length : rightIndex);
    }
