        ...
        <div className="space-y-20">
          {messages.map((msg, i) => (
            <ScrollReveal key={msg.id} delay={i * 0.15}>
              <motion.div
                className={`flex flex-col gap-8 lg:flex-row lg:items-start lg:gap-16 ${i % 2 === 1 ? "lg:flex-row-reverse" : ""
                  }`}
              >
                  ...
                  <div className="relative">
                    ...
                    <div className="prose prose-lg max-w-none text-[var(--color-muted-fg)]">
                      {(() => {
                        ...
                        const needsToggle = paragraphs.length > 2 || msg.message.length > 260;
                        ...
                        return (
                          <>
                            {displayParagraphs.map((para, pi) => (
                              <div key={pi} dangerouslySetInnerHTML={{ __html: sanitizeHtml(para) }} />
                            ))}
                            {needsToggle && (
                              <button
                                type="button"
                                onClick={() => toggleExpanded(msg.id)}
                                className="mt-3 text-sm font-semibold text-[var(--color-primary)] transition hover:underline"
                              >
